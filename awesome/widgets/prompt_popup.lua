...
function widget.new()
    local widget_instance = {
        _cached_wiboxes = {},
        --_cmd_pixelate = [[bash -c 'ffmpeg -loglevel panic -f x11grab -video_size 1920x1060 -y -i :0.0+%s,20 -vf frei0r=pixeliz0r -vframes 1 /tmp/prompt_img.png ; echo done']],
        _cmd_blur = [[ bash -c 'ffmpeg -loglevel panic -f x11grab -video_size 1920x1080 -y -i :0.0+%s -filter_complex "boxblur=9" -vframes 1 /tmp/prompt_img%s.png ; echo done' ]]
    }
    ...
    function widget_instance:launch(s, c)
        ...
        awful.spawn.with_line_callback(string.format(self._cmd_blur, tostring(awful.screen.focused().geometry.x), tostring(rnd)), {
            stdout = function(line)
                w.visible = true
                w.bgimage = ""
                w.bgimage = string.format("/tmp/prompt_img%s.png", tostring(rnd))
                ...
                awful.prompt.run {
                    ...
                    done_callback = function()
                        w.visible = false
                        awful.spawn([[bash -c 'rm -f /tmp/prompt_img*']])
                    end
                }
            end,
